...
public class CommonControllerAdvice {
	...
	public ErrorsDto handleMovieAlreadyStartedException(MovieAlreadyStartedException exception, Locale locale){
		String errorMessage = messageSource.getMessage(MOVIE_ALREADY_STARTED_EXCEPTION_CODE,
				new Object[] {exception.getMovieId(), exception.getSesionId()}, MOVIE_ALREADY_STARTED_EXCEPTION_CODE,locale);
		return new ErrorsDto(errorMessage);
	}

	// lo pongo aqui porque hay dos funciones que utilizan esta excepcion, pero no se si está bien hecha. Igual hay que ponerle
	// el sesionId o que se yo
